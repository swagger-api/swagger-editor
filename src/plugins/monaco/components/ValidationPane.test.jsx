...
const setup = ({ markerErrorList } = {}) => {
  ...
  const onValidationKeyClick = jest.fn();
  return { editorSelectors, onValidationKeyClick };
};
...
describe('with empty errorMarkerErrorList', () => {
  test('should render with table headers and no table rows', async () => {
    ...
    const { onValidationKeyClick, editorSelectors: selectors } = setup({
      markerErrorList: [],
    });
    const { hasTableItem } = await renderValidationPane({
      onValidationKeyClick,
...
describe('with populated markerErrorList', () => {
  test('should respond to clickEvent within table rows', async () => {
    ...
    expect(onValidationKeyClick).toBeCalled();
    expect(onValidationKeyClick).toBeCalledTimes(1);

    clickTableItem(columnStartLineNumber2);
    expect(onValidationKeyClick).toBeCalled();
    expect(onValidationKeyClick).toBeCalledTimes(2);

    expect(onValidationKeyClick.mock.calls[0][0].message).toBe(columnMessage1);
    expect(onValidationKeyClick.mock.calls[1][0].startLineNumber).toBe(columnStartLineNumber2);
  });
});
