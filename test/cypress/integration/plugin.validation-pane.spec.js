describe.skip('Monaco Editor with Validation Pane', () => {
  ...
  it('should display visible Validation Pane table header and table body when error exists', () => {
    ...
    cy.get('.swagger-ide__validation-table > thead > tr > :nth-child(1)')
      ...
      .should('be.visible');
    cy.get('.swagger-ide__validation-table > thead > tr > :nth-child(2)')
      .contains('description', { matchCase: false })
      .should('be.visible');
    cy.get('.swagger-ide__validation-table > tbody > :nth-child(1) > :nth-child(1) > div')
      .contains('1')
      .should('be.visible');
    cy.get('.swagger-ide__validation-table > tbody > :nth-child(1) > :nth-child(2) > div')
  ...
  it('should not display Validation Pane after error is cleared', () => {
    // fix the typo error
    cy.get('.monaco-editor textarea:first').click().focused().type(`{backspace}`);
    // re-assert
    cy.get('.swagger-ide__validation-table > thead').should('not.exist');
    cy.get('.swagger-ide__validation-table > tbody').should('not.exist');
  });
});
